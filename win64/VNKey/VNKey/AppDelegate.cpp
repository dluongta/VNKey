...
int AppDelegate::run(HINSTANCE hInstance) {
	...
	if (previousInstance) {
		MessageBeep(MB_OK);
		SendMessage(previousInstance, WM_USER + 2025, 0, 0);
		PostQuitMessage(0);
		return 0;
	}

	//init VNKey Engine
	VNKeyManager::initEngine();
	...
	while (GetMessage(&msg, nullptr, 0, 0))	{
		if (msg.message == WM_KEYDOWN) {
			VNKeyManager::_lastKeyCode = (UINT16)msg.wParam;
		}
		if (!isDialogMsg(msg)) {
...
void AppDelegate::onToggleVietnamese() {
	...
	if (vUseSmartSwitchKey) {
		string& exe = VNKeyHelper::getLastAppExecuteName();
...
void AppDelegate::onVNKeyExit() {
	VNKeyManager::freeEngine();
