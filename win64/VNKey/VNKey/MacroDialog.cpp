...
void MacroDialog::saveAndReload() {
	...
	VNKeyHelper::setRegBinary(_T("macroData"), macroData.data(), (int)macroData.size());
...
void MacroDialog::onAddMacroButton() {
	...
	if (name.compare(L"") == 0 || content.compare(L"") == 0) {
		MessageBox(hDlg, _T("Bạn hãy nhập từ cần gõ tắt!"), _T("VNKey"), MB_OK);
		return;
	}
...
void MacroDialog::onDeleteMacroButton() {
	...
	if (name.compare(L"") == 0) {
		MessageBox(hDlg, _T("Bạn hãy chọn từ cần xoá!"), _T("VNKey"), MB_OK);
		return;
	}
