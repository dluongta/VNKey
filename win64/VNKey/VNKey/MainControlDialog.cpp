...
void MainControlDialog::initDialog() {
    ...
    wsprintfW(titleBuffer, title, VNKeyHelper::getVersionString().c_str());
    SetWindowText(hDlg, titleBuffer);

    //input type
    comboBoxInputType = GetDlgItem(hDlg, IDC_COMBO_INPUT_TYPE);
    vector<LPCTSTR>& inputType = VNKeyManager::getInputType();
    ...
    vector<LPCTSTR>& tableCode = VNKeyManager::getTableCode();
...
INT_PTR MainControlDialog::eventProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND: {
        ...
        case IDC_BUTTON_GO_SOURCE_CODE:
            ShellExecute(NULL, _T("open"), _T(" "), NULL, NULL, SW_SHOWNORMAL);
            break;
        default:
            if (HIWORD(wParam) == CBN_SELCHANGE) {
                ...
                _lastKeyCode = VNKeyManager::_lastKeyCode;
                if (_lastKeyCode > 0) {
                    VNKeyManager::_lastKeyCode = 0;
...
void MainControlDialog::fillData() {
    ...
    SendMessage(checkTempOffVNKey, BM_SETCHECK, vTempOffVNKey ? 1 : 0, 0);
    ...
    //tab info
    wchar_t buffer[256];
    wsprintfW(buffer, _T("Phiên bản %s cho Windows - Ngày cập nhật: %s"), VNKeyHelper::getVersionString().c_str(), _T(__DATE__));
...
void MainControlDialog::onComboBoxSelected(const HWND& hCombobox, const int& comboboxId) {
    if (hCombobox == comboBoxInputType) {
        ...
        if (vRememberCode) {
            setAppInputMethodStatus(VNKeyHelper::getFrontMostAppExecuteName(), vLanguage | (vCodeTable << 1));
...
void MainControlDialog::onCheckboxClicked(const HWND& hWnd) {
    ...
    if (hWnd == checkCtrl) {
        ...
        VNKeyHelper::registerRunOnStartup(vRunWithWindows);
    }
        ...
        if (val)
            VNKeyManager::createDesktopShortcut();
    }
    ...
    else if (hWnd == checkTempOffVNKey) {
        val = (int)SendMessage(hWnd, BM_GETCHECK, 0, 0);
        APP_SET_DATA(vTempOffVNKey, val ? 1 : 0);
    }
...
void MainControlDialog::onUpdateButton() {
    ...
    string newVersion;
    if (VNKeyManager::checkUpdate(newVersion)) {
        WCHAR msg[256];
        wsprintf(msg,
            TEXT("VNKey Có phiên bản mới (%s), bạn có muốn cập nhật không?"),
        ...
        if (msgboxID == IDYES) {
            ...
            wsprintf(path, TEXT("%s\\VNKeyUpdate.exe"), path);
        ...
        MessageBox(hDlg, _T("Bạn đang dùng phiên bản mới nhất!"), _T("VNKey Update"), MB_OK);
    }
    EnableWindow(hUpdateButton, true);
}

void MainControlDialog::requestRestartAsAdmin() {
    VNKeyHelper::registerRunOnStartup(false);
    if (vRunAsAdmin && !IsUserAnAdmin()) {
        int msgboxID = MessageBox(
            hDlg,
            _T("Bạn cần phải khởi động lại VNKey để kích hoạt chế độ Admin!\nBạn có muốn khởi động lại VNKey không?"),
            _T("VNKey"),
            MB_ICONEXCLAMATION | MB_YESNO
        );
        if (msgboxID == IDYES) {
