...
static LPCTSTR sk = TEXT("SOFTWARE\\TuyenMai\\VNKey");
static HKEY hKey;
...
static HANDLE _proc;
static string _exeNameUtf8 = "TheVNKeyProject";
...
void VNKeyHelper::setRegInt(LPCTSTR key, const int & val) {
	openKey();
	RegSetValueEx(hKey, key, 0, REG_DWORD, (LPBYTE)&val, sizeof(val));
	RegCloseKey(hKey);
}

int VNKeyHelper::getRegInt(LPCTSTR key, const int & defaultValue) {
	openKey();
...
void VNKeyHelper::setRegBinary(LPCTSTR key, const BYTE * pData, const int & size) {
	openKey();
	RegSetValueEx(hKey, key, 0, REG_BINARY, pData, size);
	RegCloseKey(hKey);
}

BYTE * VNKeyHelper::getRegBinary(LPCTSTR key, DWORD& outSize) {
	openKey();
	if (_regData) {
		delete[] _regData;
		_regData = NULL;
...
void VNKeyHelper::registerRunOnStartup(const int& val) {
	if (val) {
		if (vRunAsAdmin) {
			string path = wideStringToUtf8(getFullPath());
			char buff[MAX_PATH];
			sprintf_s(buff, "schtasks /create /sc onlogon /tn VNKey /rl highest /tr \"%s\" /f", path.c_str());
			WinExec(buff, SW_HIDE);
		} else {
			RegOpenKeyEx(HKEY_CURRENT_USER, _runOnStartupKeyPath, NULL, KEY_ALL_ACCESS, &hKey);
			wstring path = getFullPath();
			RegSetValueEx(hKey, _T("VNKey"), 0, REG_SZ, (byte*)path.c_str(), ((DWORD)path.size() + 1) * sizeof(TCHAR));
			RegCloseKey(hKey);
		}
	} else {
		RegOpenKeyEx(HKEY_CURRENT_USER, _runOnStartupKeyPath, NULL, KEY_ALL_ACCESS, &hKey);
		RegDeleteValue(hKey, _T("VNKey"));
		RegCloseKey(hKey);
		WinExec("schtasks /delete  /tn VNKey /f", SW_HIDE);
	}
}

LPTSTR VNKeyHelper::getExecutePath() {
	if (!_hasGetPath) {
...
string& VNKeyHelper::getFrontMostAppExecuteName() {
	...
	if (wcscmp(_exeName, _T("VNKey64.exe")) == 0 ||
		wcscmp(_exeName, _T("VNKey32.exe")) == 0 || 
...
string & VNKeyHelper::getLastAppExecuteName() {
	if (!vUseSmartSwitchKey)
		return getFrontMostAppExecuteName();
	return _exeNameUtf8;
}

wstring VNKeyHelper::getFullPath() {
...
wstring VNKeyHelper::getClipboardText(const int& type) {
...
void VNKeyHelper::setClipboardText(LPCTSTR data, const int & len, const int& type) {
...
DWORD VNKeyHelper::getVersionNumber() {
...
wstring VNKeyHelper::getVersionString() {
...
wstring VNKeyHelper::getContentOfUrl(LPCTSTR url){
	WCHAR path[MAX_PATH];
	GetTempPath2(MAX_PATH, path);
	wsprintf(path, TEXT("%s\\_VNKey.tempf"), path);
